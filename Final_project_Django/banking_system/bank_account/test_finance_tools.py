...
class TestCalculateEMI(unittest.TestCase):
    def test_input_validation(self):
        ...
        with self.assertRaises(ValueError):
            calculate_emi(100000, 7.5, 0)
        with self.assertRaises(TypeError):
            calculate_emi("loan", 7.5, 10)
...
class SIPCalculation(unittest.TestCase):
    ...
    def test_zero_investment_or_years(self):
        with self.assertRaises(ValueError):
            calculate_sip(0, 12, 10)

        with self.assertRaises(ValueError):
            calculate_sip(5000, 12, 0)
...
class RetirementEstimationTests(unittest.TestCase):
    ...
    def test_valid_inputs_and_output(self):
        ...
        self.assertGreaterEqual(total_corpus, 0)
        self.assertGreaterEqual(total_invested, 0)
        self.assertGreaterEqual(total_interest, 0)
        ...
        self.assertTrue(total_invested, expected_total_invested)
        self.assertTrue(total_interest, expected_total_interest)
...
class TestCalculateCreditCardBalance(unittest.TestCase):
    ...
    def test_invalid_inputs(self):
        ...
        with self.assertRaises(ValueError):
            calculate_credit_card_balance(1000, 18, -100, 12)
        with self.assertRaises(ValueError):
            calculate_credit_card_balance(1000, 18, 100, 0)
        with self.assertRaises(ValueError):
            calculate_credit_card_balance(1000, 18, 100, -12)
        with self.assertRaises(TypeError):
            calculate_credit_card_balance("1000", 18, 100, 12)
...
class TestCalculateTaxableIncome(unittest.TestCase):
    ...
    def test_zero_income(self):
        self.assertEqual(calculate_taxable_income(0, 0), 0.00)

    def test_rounding(self):
        self.assertEqual(calculate_taxable_income(100000.567, 20000.333), 30000.23 )
...
class TestBudgetPlanner(unittest.TestCase):
    ...
    def test_negative_surplus(self):
        result = budget_planner(40000, 50000)
        self.assertEqual(result['surplus'], -10000)
        self.assertEqual(result['savings'], 0)
        self.assertEqual(result['investments'], 0)
        self.assertEqual(result['free_to_use'], 0)
        self.assertIn("overspending", result['message'])
    ...
    def test_rounding(self):
        result = budget_planner(12345.67, 10000.23)
        self.assertEqual(result['surplus'], 2345.44)
        self.assertEqual(result['savings'], round(2345.44 * 0.2, 2))
        self.assertEqual(result['investments'], round(2345.44 * 0.3, 2))
        self.assertEqual(result['free_to_use'], round(2345.44 * 0.5, 2))
...
class TestCalculateNetWorth(unittest.TestCase):
    ...
    def test_rounding(self):
        self.assertEqual(calculate_net_worth(123456.789, 23456.789), 100000.0)

    def test_large_values(self):
        self.assertEqual(calculate_net_worth(1e9, 5e8), 500000000.00)
